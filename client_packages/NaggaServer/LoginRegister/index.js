...
mp.events.add('onPlayerConnected', () => {
	...
	welcomePage = mp.browsers.new("package://NaggaServer/LoginRegister/LoginRegister.html");
...
mp.events.add('onPlayerRegisterResponse', (success) => {
	welcomePage.execute(`$(\"#registerResponseMessage\").find(\"#registerMessage\").remove();`)
	if (success) {
		welcomePage.execute(`var alertElement = $(\'<div id="registerMessage" class="alert alert-success">Contul a fost creat, te rugam sa te autentifici!</div >\'); \
		$(\"#registerResponseMessage\").append(alertElement);`);
	} else {
		welcomePage.execute(`var alertElement = $(\'<div id="registerMessage" class="alert alert-danger">Parola sau Username-ul sunt gresite.</div >\'); \
            $(\"#registerResponseMessage\").append(alertElement);`);
	}
});